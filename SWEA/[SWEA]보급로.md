```python
'''
가장 작은 경로를 찾기 위해 해당 위치에서 상, 하, 좌, 우 탐색을 해야 하므로 BFS로 풀어야 한다. 
경로를 탐색하면서 더 적은 비용이라면 CHECK[now_dr][now_dc]를 갱신하면서 MAP을 탐색한다. 
계속 적은 값을 갱신하기 때문에 CHECK[N-1][N-1]에는 가장 적은 비용의 경로가 저장되어 있게 된다.
'''
def bfs():     # BFS 탐색
    dr = [-1, 1, 0, 0]
    dc = [0, 0, -1, 1]
